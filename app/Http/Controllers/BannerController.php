...
class BannerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // $uploadedFile = copy($upload_path.$bannerImageSaveAsName, $all_upload_path.$bannerImageSaveAsName);
        ...
        $allImagesUpload = Image::create(['name'=>$bannerImageSaveAsName, 'path'=>$backupDisk]);
        // $allImagesUpload = Image::create(['name'=>$bannerImageSaveAsName, 'path'=>$all_image_url]);



        $result = Banner::insert(['title'=>$name,'slug'=>$slug,'photo'=>$url,'img_name'=>$path,'description'=>$desc,'status'=>$status]);
        // $result = Banner::insert(['title'=>$name,'slug'=>$slug,'photo'=>$banner_image_url,'description'=>$desc,'status'=>$status]);
        if ($result==true) {
          return 1;
        } else {
          return 0;
    ...
    public function updateBanner(Request $request)
    {
      ...
      $count=Banner::where('slug',$slug)->count();
        ...
        if($request->hasFile('img')){
          ...
          $allImagesUpload = Image::create(['name'=>$bannerImageSaveAsName, 'path'=>$backupUpdateDisk]);
          // $allImagesUpload = Image::create(['name'=>$bannerImageSaveAsName, 'path'=>$all_image_url]);
          ...
          // $banner_image_url =$findUbcat->photo;
        }


      $result= Banner::where('id','=',$id)->update([
        'title'=>$name,'slug'=>$slug,'photo'=>$Updateurl, 'img_name'=>$updatepath, 'description'=>$desc,'status'=>$status
       ]);
      // $result= Banner::where('id','=',$id)->update([
      //   'title'=>$name,'slug'=>$slug,'photo'=>$banner_image_url,'description'=>$desc,'status'=>$status
      //  ]);
